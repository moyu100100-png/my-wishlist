...
const ListIcon=()=><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" style={{width:20,height:20}}><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="18" x2="21" y2="18"/></svg>;
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',height:'100dvh',background:'var(--bg)'}}>
      ...
      <div style={{background:'var(--ivory)',borderBottom:'1px solid var(--border2)',padding:'0 16px',height:40,display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
        <span style={{fontSize:11,color:'var(--t3)'}}>{colls.length} items</span>
        <div style={{display:'flex',gap:4}}>
          <button style={IB}><FilterIcon/></button>
          <button style={IB}><SortIcon/></button>
      ...
      <div style={{flex:1,overflowY:'auto'}}>
        {!colls.length?(
          <div style={{textAlign:'center',padding:'60px 20px',color:'var(--t3)'}}><p style={{fontSize:13,lineHeight:2.2}}>まだ Collection がありません。<br/>Wish から移動しましょう。</p></div>
        ):view==='grid'?(
            ...
            {colls.map(item=>{
              ...
              return(
                <div key={item.id} onClick={()=>setDetailId(item.id)} style={{background:'var(--surface)',borderRadius:14,border:'1px solid var(--border2)',display:'flex',gap:12,padding:12,marginBottom:10,cursor:'pointer'}}>
                  <div style={{width:64,height:64,borderRadius:10,overflow:'hidden',flexShrink:0,background:'var(--ivory2)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,color:'var(--t3)'}}>
                    {item.images.length?<img src={item.images[0]} alt={item.name} style={{width:'100%',height:'100%',objectFit:'cover'}}/>:item.brand.slice(0,3).toUpperCase()}
                  </div>
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontFamily:'Cormorant Garamond,serif',fontSize:15,marginBottom:4}}>{fmt(item.purchasePrice)}</div>
                    <div style={{fontSize:11,color:'var(--t3)'}}><strong style={{color:'var(--t1)',fontWeight:400}}>{days}d</strong> · <strong style={{color:'var(--t1)',fontWeight:400}}>{fmt(perDay)}</strong>/day</div>
                  </div>
                </div>
              );

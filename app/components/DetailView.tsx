...
export default function DetailView({itemId,onClose,onEdit,onUpdate,allItems=[]}:Props){
  ...
  const cd=store.getColors().find(c=>c.hex===item.color)||{hex:item.color,shimmer:false};
      ...
      <div style={{position:'fixed',inset:0,zIndex:100,overflowY:'auto',display:'flex',flexDirection:'column',background:'var(--ivory)'}}
        ...
        <div style={{position:'relative',background:'var(--ivory2)',flexShrink:0}}>
          ...
          <div style={{position:'absolute',top:0,left:0,right:0,padding:'12px 14px',display:'flex',justifyContent:'space-between',background:'linear-gradient(to bottom,rgba(249,247,242,.88),transparent)'}}>
            <button onClick={onClose} style={{width:36,height:36,display:'flex',alignItems:'center',justifyContent:'center',borderRadius:'50%',background:'rgba(249,247,242,.88)',cursor:'pointer'}}>
              <svg viewBox="0 0 24 24" fill="none" stroke="var(--brown)" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" style={{width:19,height:19}}><polyline points="15 18 9 12 15 6"/></svg>
            </button>
            <div style={{position:'relative'}}>
              <button onClick={()=>setMenuOpen(p=>!p)} style={{width:36,height:36,display:'flex',alignItems:'center',justifyContent:'center',borderRadius:'50%',background:'rgba(249,247,242,.88)',cursor:'pointer'}}>
                <svg viewBox="0 0 24 24" fill="none" stroke="var(--brown)" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" style={{width:19,height:19}}><circle cx="12" cy="12" r="1"/><circle cx="19" cy="12" r="1"/><circle cx="5" cy="12" r="1"/></svg>
              </button>
              {menuOpen&&<div style={{position:'absolute',top:40,right:0,background:'var(--surface)',border:'1px solid var(--border2)',borderRadius:12,boxShadow:'0 4px 16px rgba(0,0,0,.1)',minWidth:120,zIndex:10}}>
                <button onClick={()=>{setMenuOpen(false);onEdit(item);}} style={{width:'100%',padding:'12px 16px',fontSize:13,textAlign:'left',color:'var(--t1)',cursor:'pointer',borderBottom:'1px solid var(--border2)'}}>編集</button>
                <button onClick={()=>{setMenuOpen(false);if(!confirm('削除しますか？'))return;if(isColl)store.saveColls(store.getColls().filter(i=>i.id!==item.id));else store.saveItems(store.getItems().filter(i=>i.id!==item.id));onUpdate();onClose();}} style={{width:'100%',padding:'12px 16px',fontSize:13,textAlign:'left',color:'#C0392B',cursor:'pointer'}}>削除</button>
              </div>}
            </div>
          </div>
          ...
          {allItems.length>1&&<>
            {curIdx>0&&<button onClick={()=>navItem('prev')} style={{position:'absolute',left:12,top:'50%',transform:'translateY(-50%)',width:32,height:32,borderRadius:'50%',background:'rgba(249,247,242,.88)',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}}>
              <svg viewBox="0 0 24 24" fill="none" stroke="var(--brown)" strokeWidth="1.6" strokeLinecap="round" style={{width:16,height:16}}><polyline points="15 18 9 12 15 6"/></svg>
            </button>}
            {curIdx<allItems.length-1&&<button onClick={()=>navItem('next')} style={{position:'absolute',right:12,top:'50%',transform:'translateY(-50%)',width:32,height:32,borderRadius:'50%',background:'rgba(249,247,242,.88)',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}}>
              <svg viewBox="0 0 24 24" fill="none" stroke="var(--brown)" strokeWidth="1.6" strokeLinecap="round" style={{width:16,height:16}}><polyline points="9 18 15 12 9 6"/></svg>
            </button>}
          </>}
        </div>
        {/* Body */}
          ...
          {isColl?(
            <>
              <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:18}}>
                <div><div style={{fontFamily:'Cormorant Garamond,serif',fontSize:22,fontWeight:400,lineHeight:1.2}}>{item.brand}</div><div style={{fontSize:13,color:'var(--t2)',marginTop:3}}>{item.name}</div></div>
                <span style={{fontSize:20,color:'var(--t3)'}}>♡</span>
              </div>
              <table style={{width:'100%',borderCollapse:'collapse',marginBottom:18}}><tbody>
                <tr style={TR}><td style={td1}>購入日</td><td style={td2}>{coll!.purchaseDate}</td></tr>
                <tr style={TR}><td style={td1}>購入価格</td><td style={td2}>{fmt(coll!.purchasePrice)}</td></tr>
                <tr><td style={td1}>登録価格</td><td style={td2}>{fmt(item.price)}</td></tr>
              </tbody></table>
              <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:1,background:'var(--border2)',borderRadius:12,overflow:'hidden',marginBottom:22}}>
                {[{l:'差額',v:fmt(diff),c:diff>=0?'#5C8C6A':'#A05050'},{l:'保有日数',v:`${days}日`,c:'var(--t1)'},{l:'1日あたりのコスト',v:fmt(perDay),c:'var(--t1)'}].map(s=>(
                  <div key={s.l} style={{background:'var(--ivory2)',padding:'13px 10px'}}>
                    <div style={{fontSize:9,color:'var(--t3)',letterSpacing:'.06em',marginBottom:6,textTransform:'uppercase'}}>{s.l}</div>
                    <div style={{fontFamily:'Cormorant Garamond,serif',fontSize:18,color:s.c}}>{s.v}</div>
                  </div>
                ))}
              </div>
              {item.memo&&<div style={{marginBottom:18}}><div style={{fontSize:12,color:'var(--t3)',marginBottom:5,letterSpacing:'.06em',textTransform:'uppercase'}}>Memo</div><div style={{fontSize:13,lineHeight:1.8}}>{item.memo}</div></div>}
              <PriceChart history={item.priceHistory}/>
              <div style={{borderTop:'1px solid var(--border2)',margin:'18px 0'}}/>
              <button onClick={()=>{setPuPrice(String(item.currentPrice||item.price));setPuOpen(true);}} style={{width:'100%',padding:12,background:'none',border:'1px solid var(--brown)',borderRadius:24,fontSize:13,color:'var(--brown)',cursor:'pointer',letterSpacing:'.04em'}}>現在価格を更新</button>
            </>
          ):(
            <>
              ...
              <div style={{marginBottom:14}}>
                <div style={{fontFamily:'Cormorant Garamond,serif',fontSize:30,fontWeight:300}}>{fmt(item.currentPrice||item.price)}</div>
              ...
              <div style={{marginBottom:18}}><StarRating value={item.priority} readonly size={18}/></div>
              <div style={{borderTop:'1px solid var(--border2)',marginBottom:16}}/>
              <table style={{width:'100%',borderCollapse:'collapse',marginBottom:18}}><tbody>
                <tr style={TR}><td style={td1}>カテゴリ</td><td style={td2}>{item.cat||'—'}</td></tr>
                <tr style={TR}><td style={td1}>カラー</td><td style={td2}><span style={{display:'inline-flex',alignItems:'center',gap:6}}><span style={{width:13,height:13,borderRadius:'50%',...swatchStyle(cd),display:'inline-block',flexShrink:0}}/>{item.color}</span></td></tr>
                <tr style={TR}><td style={td1}>登録価格</td><td style={td2}>{fmt(item.price)}</td></tr>
                <tr style={TR}><td style={td1}>現在価格</td><td style={td2}>{item.currentPrice?fmt(item.currentPrice):'—'}<button onClick={()=>{setPuPrice(String(item.currentPrice||item.price));setPuOpen(true);}} style={{fontSize:11,color:'var(--brown)',marginLeft:6,textDecoration:'underline',textUnderlineOffset:2,cursor:'pointer'}}>更新</button></td></tr>
                {item.url&&<tr style={TR}><td style={td1}>URL</td><td style={td2}><a href={item.url} target="_blank" rel="noreferrer" style={{color:'var(--brown)',textDecoration:'underline',textUnderlineOffset:2}}>リンクを開く</a></td></tr>}
                <tr><td style={td1}>登録日</td><td style={td2}>{item.date}</td></tr>
              </tbody></table>
              <PriceChart history={item.priceHistory}/>
              <div style={{borderTop:'1px solid var(--border2)',margin:'18px 0'}}/>

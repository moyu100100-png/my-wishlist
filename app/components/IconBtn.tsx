...
export default function FilterSheet({open,onClose,current,onApply}:Props){
  ...
  return(
    <Sheet open={open} onClose={onClose} title="フィルター"
      rightAction={<button onClick={()=>{setF(defaultFilter);onApply(defaultFilter);onClose();}} style={{fontSize:12,color:'var(--brown)',cursor:'pointer',background:'none',border:'none',padding:4}}>リセット</button>}>
      <div style={{paddingBottom:32}}>
        <div style={{padding:'16px 20px 0'}}><p style={ST}>カテゴリ</p><div style={{display:'flex',flexWrap:'wrap',gap:8}}>
          <button style={chip(!f.cats.length)} onClick={()=>setF(p=>({...p,cats:[],colors:[],brands:[]}))}>すべて</button>
          {allCats.map(c=><button key={c} style={chip(f.cats.includes(c))} onClick={()=>setF(p=>({...p,cats:tog(p.cats,c)}))}>{c}</button>)}
        </div></div>
        <div style={{padding:'16px 20px 0'}}><p style={ST}>カラー</p><div style={{display:'flex',flexWrap:'wrap',gap:10}}>
          {allColors.map(c=>{const dis=f.cats.length&&!availColors.includes(c.hex);return(
            <button key={c.hex} onClick={()=>!dis&&setF(p=>({...p,colors:tog(p.colors,c.hex)}))} title={c.hex}
              style={{width:34,height:34,borderRadius:'50%',background:c.hex,cursor:dis?'not-allowed':'pointer',flexShrink:0,outline:f.colors.includes(c.hex)?'2.5px solid var(--brown)':'none',outlineOffset:3,opacity:dis?.3:1,border:['#FFFFFF','#E8E8E8','#C0C0C0'].includes(c.hex)?'1px solid var(--border)':'none',...(c.shimmer?{backgroundImage:'linear-gradient(135deg,rgba(255,255,255,.65) 0%,rgba(255,255,255,.0) 45%,rgba(255,255,255,.45) 100%)'}:{})}}/>
          );})}
        </div></div>
        <div style={{padding:'16px 20px 0'}}><p style={ST}>ブランド</p><div style={{display:'flex',flexWrap:'wrap',gap:8}}>
          {allBrands.map(b=>{const dis=f.cats.length&&!availBrands.includes(b);return(
            <button key={b} style={chip(f.brands.includes(b),dis)} onClick={()=>!dis&&setF(p=>({...p,brands:tog(p.brands,b)}))}>{b}</button>
          );})}
        </div></div>

...
export default function IconBtn({onClick,children,active=false,label,size=36}:Props){
  return(
    <button onClick={onClick} title={label} style={{
      width:size,height:size,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:2,
      borderRadius:10,cursor:'pointer',
      background:active?'var(--brown-bg)':'var(--surface)',
      border:'1px solid var(--border2)',
      boxShadow:'0 1px 4px rgba(160,135,106,.15)',
      color:active?'var(--brown)':'var(--t2)',

...
export default function AddItemSheet({open,onClose,editItem,onSave}:Props){
  ...
  const [cats,setCats]=useState<string[]>([]);const [colors,setColors]=useState<any[]>([]);
  ...
  useEffect(()=>{
    if(open){
      ...
      else{setBrand('');setName('');setCat('');setColor('');setPrice('');setUrl('');setMemo('');setPriority(3);setImages([]);}
    }
  },[open,editItem]);
  ...
  function submit(){
    ...
    }else{const items=store.getItems();items.unshift({id:uid(),currentPrice:null,priceHistory:[],date:new Date().toISOString().slice(0,10),...data});store.saveItems(items);}
    onSave();onClose();
  }
    ...
    <Sheet open={open} onClose={onClose} title={editItem?'アイテムを編集':'アイテムを登録'}>
      <div style={{display:'flex',flexDirection:'column',gap:16,padding:'16px 20px 40px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
          <div><label style={L}>ブランド *</label><input style={I} value={brand} onChange={e=>setBrand(e.target.value)} placeholder="Cartier"/></div>
          <div><label style={L}>カテゴリ</label><select style={I} value={cat} onChange={e=>setCat(e.target.value)}>{cats.map(c=><option key={c} value={c}>{c}</option>)}</select></div>
        </div>
        <div><label style={L}>商品名 *</label><input style={I} value={name} onChange={e=>setName(e.target.value)} placeholder="LOVE Ring"/></div>
        <div><label style={L}>カラー</label><SwatchGrid colors={colors} selected={color} onSelect={setColor} size={32}/></div>
